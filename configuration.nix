...
{ 
  ...
  users.users.komron = {
    ...
    home = "/home/komron";
    description = "Komron";
    extraGroups = [ "wheel" ]; # gives sudo
    shell = pkgs.bash;
    initialPassword = "changeme"; # optional
  };
  ...
  # enabling cursor
  environment.systemPackages = [ pkgs.bash pkgs.git pkgs.nodejs pkgs.coreutils 
    pkgs.findutils pkgs.gnused pkgs.gnutar pkgs.gzip pkgs.openssh
  ];

  # tailscale
  services.tailscale = { enable = true; useRoutingFeatures = "both";
  };
  ...
  # {
  #   font = "Lat2-Terminus16"; keyMap = "us"; useXkbConfig = true; # use xkb.options in 
  #   tty.
  # };
  ...
  # List packages installed in system profile. You can use https://search.nixos.org/ to 
  # find more packages (and options). environment.systemPackages = with pkgs; [
  #   vim # Do not forget to add an editor to edit configuration.nix! The Nano editor is 
  #   also installed by default. wget
  # ];
  ...
  # (/run/current-system/configuration.nix). This is useful in case you accidentally 
  # delete configuration.nix. system.copySystemConfiguration = true;
